...
class CycleGANModel(BaseModel):
	...
	#g.view()
        ...
        self.netD_A = networks.define_D(opt.output_nc, opt.ndf,opt.which_model_netD,opt.n_layers_D, opt.norm, True, opt.init_type, self.gpu_ids)
        self.netD_B1=networks.define_D(opt.input_nc, opt.ndf,opt.which_model_netD,opt.n_layers_D, opt.norm, True, opt.init_type, self.gpu_ids)
        self.netD_B2=networks.define_D(opt.input_nc, opt.ndf,opt.which_model_netD,opt.n_layers_D, opt.norm, True, opt.init_type, self.gpu_ids)
    ...
    def test(self):
        ...
        self.fake_B, latent_fB, self.intermediate_fakeB = self.netG_A.forward(self.real_A1, self.real_A2)
        self.int_A1, self.int_A2,self.rec_A1, self.rec_A2, latent_rA, self.intermediate_realA  = self.netG_B.forward(self.fake_B)
   
        self.real_B = Variable(self.input_B, volatile=True)
        self.int_fake_A1,self.int_fake_A2,self.fake_A1,self.fake_A2, latent_fA, self.intermediate_fakeA = self.netG_B.forward(self.real_B)
        self.rec_B, latent_rB, self.intermediate_realB = self.netG_A.forward(self.fake_A1,self.fake_A2)
    ...
    def backward_G(self):
        ...
        # L3 loss
        self.int_A1, self.int_A2,self.rec_A1, self.rec_A2, latent_rA, self.intermediate_realA  = self.netG_B.forward(self.fake_B)
        self.loss_L3 = (self.criterionCycle(self.rec_A1, self.real_A1) * lambda_L3+self.criterionCycle(self.rec_A2,self.real_A2) * lambda_L3)
        
        #L1 Loss
        self.loss_L1 = (self.criterionCycle(self.int_A1, self.real_A1) * lambda_L1+self.criterionCycle(self.int_A2,self.real_A2) * lambda_L1)
        ...
        self.loss_L2 = lambda_L2*self.criterionNew(self.intermediate_fakeB,self.intermediate_realA) + lambda_L2*self.criterionNew(self.intermediate_fakeA,self.intermediate_realB)
        #self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + self.loss_idt_A + self.loss_idt_B + self.latent_loss + self.new_loss
        
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_L1 + self.loss_L2 + self.loss_L3 + self.loss_idt_A + self.loss_idt_B + self.latent_loss
    ...
    def get_current_errors(self):
        ...
        L1 = self.loss_L1.data.item()
        L2 = self.loss_L2.data.item()
        L3 = self.loss_L3.data.item()
        D_B = self.loss_D_B.data.item() 
        G_B = self.loss_G_B.data.item()
        Cyc_B = self.loss_cycle_B.data.item()
        #new = self.new_loss.data.item()
        if self.opt.identity > 0.0:
            ...
            return OrderedDict([('D_A', D_A), ('G_A', G_A), ('Cyc_A', Cyc_A), ('idt_A', idt_A),
                                ('D_B', D_B), ('G_B', G_B), ('idt_B', idt_B),('new_loss', new)])
        else:
            #return OrderedDict([('D_A', D_A), ('G_A', G_A), ('Cyc_A', Cyc_A),
                                #('D_B', D_B), ('G_B', G_B), ('Cyc_B', Cyc_B),('new_loss', new)])
            return OrderedDict([ ('L1', L1),('L2', L1),('L3', L3),('Cyc_B', Cyc_B),
                                ('D_B', D_B), ('G_B', G_B),('D_A', D_A), ('G_A', G_A)])
